...
module Document = struct
  ...
  let of_file ~filename =
    ...
    >>|? fun contents ->
    let doc = Ocaml_topexpect.Raw.v ~fname:filename contents in
    let phrases = Ocaml_topexpect.Phrase.read_all doc in
    Ocaml_topexpect.Phrase.document_of_phrases doc true phrases
